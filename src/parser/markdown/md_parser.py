...
def build_blocks(lines):
    ...
    for line in lines:
        ...
        if kind == "TEXT":
            if current_block and current_block["block_type"] in ("paragraph", "list_item", "quote"):
                # Append to current block if no blank line separates them
                current_block["lines"].append(line)
                current_block["end_line"] = line["line_number"]
            else:
                flush_block()
        ...
        # For single-line or multi-line structures (HEADING, LIST_ITEM, TABLE_ROW, QUOTE)
        flush_block()
        current_block = {
...
def _parse_heading_level_and_text(text):
    ...
    if m:
        return max(2, len(m.group(1))), m.group(2).strip()
    return 2, stripped
...
def to_parsed_blocks(raw_blocks, document_id):
    ...
    for idx, raw in enumerate(raw_blocks, start=1):
        ...
        block_id = compute_block_id(
            document_id, "markdown", btype, canonical_locator, canonical_native_content
        )
...
def parse_md(file_path, document_id, output_dir):
